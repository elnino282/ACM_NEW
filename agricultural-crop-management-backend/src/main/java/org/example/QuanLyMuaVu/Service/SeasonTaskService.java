...
public class SeasonTaskService {
    ...
    public TaskResponse updateTask(Integer id, UpdateTaskRequest request) {
        ...
        LocalDate planned = request.getPlannedDate();
        LocalDate due = request.getDueDate();
        validateTaskDatesWithinSeason(task.getSeason(), planned, due, task.getActualStartDate(), task.getActualEndDate());
    ...
    public TaskResponse updateTaskStatus(Integer id, UpdateTaskStatusRequest request) {
        ...
        validateTaskDatesWithinSeason(task.getSeason(), task.getPlannedDate(), task.getDueDate(), actualStart, actualEnd);
    ...
    private boolean isValidStatusTransition(TaskStatus currentStatus, TaskStatus targetStatus) {
        ...
        return switch (currentStatus) {
            case PENDING -> EnumSet.of(TaskStatus.IN_PROGRESS, TaskStatus.DONE, TaskStatus.CANCELLED, TaskStatus.OVERDUE)
                    .contains(targetStatus);
            case IN_PROGRESS -> EnumSet.of(TaskStatus.DONE, TaskStatus.CANCELLED, TaskStatus.OVERDUE).contains(targetStatus);
