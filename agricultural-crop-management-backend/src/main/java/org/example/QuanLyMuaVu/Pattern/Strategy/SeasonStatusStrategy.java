...
public class SeasonStatusStrategy implements StatusTransitionStrategy<SeasonStatus> {
    ...
    static {
        ...
        TRANSITIONS.put(SeasonStatus.ACTIVE, EnumSet.of(SeasonStatus.COMPLETED, SeasonStatus.CANCELLED));

        // COMPLETED is terminal (but can be cancelled for auditing purposes)
        TRANSITIONS.put(SeasonStatus.COMPLETED, EnumSet.of(SeasonStatus.CANCELLED));

        // CANCELLED is terminal - no further transitions
        TRANSITIONS.put(SeasonStatus.CANCELLED, EnumSet.noneOf(SeasonStatus.class));
    }
    ...
    public boolean isTerminalStatus(SeasonStatus status) {
        return status == SeasonStatus.CANCELLED;
    }
