...
export function RecentActivityTimeline({
  ...
  return (
    <Card className="border-[#E0E0E0] rounded-2xl shadow-sm mt-6">
      <CardHeader>
          ...
          {activities.map((activity, index) => (
            <div key={activity.id} className="flex items-start gap-4">
              <div className="relative">
                <div className="w-8 h-8 rounded-full bg-white border-2 border-[#E0E0E0] flex items-center justify-center">
                  {getActivityIcon(activity.type)}
                </div>
                {index < activities.length - 1 && (
                  <div className="absolute left-4 top-8 w-0.5 h-8 bg-[#E0E0E0]"></div>
                )}
              </div>
