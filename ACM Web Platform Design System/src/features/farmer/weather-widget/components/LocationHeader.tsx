...
export const LocationHeader = React.memo<LocationHeaderProps>(
    ({
        ...
        return (
            <div className="bg-gradient-to-r from-[#4A90E2] to-[#4A90E2]/80 px-4 py-3 relative">
                            ...
                            <div className="flex items-center gap-2 flex-1 relative">
                                <div className="relative flex-1">
                                    <Input
                                        ...
                                        className="h-7 text-sm bg-white/20 border-white/30 text-white placeholder:text-white/70 focus:bg-white/30"
                                    ...
                                    {(locationSuggestions.length > 0 || isSearchingLocations) && (
                                        <div
                                            ref={suggestionsRef}
                                            className="absolute top-full left-0 right-0 mt-1 bg-white rounded-md shadow-lg border border-[#E0E0E0] z-50 max-h-[200px] overflow-y-auto"
                                        >
                                            {isSearchingLocations ? (
                                                <div className="px-3 py-2 text-sm text-[#6B7280] flex items-center gap-2">
                                                ...
                                                locationSuggestions.map((suggestion, index) => (
                                                    <button
                                                        ...
                                                        className={`w-full text-left px-3 py-2 text-sm hover:bg-[#4A90E2]/10 transition-colors ${
                                                            index === selectedIndex
                                                                ? "bg-[#4A90E2]/10"
                                                                : ""
                                                        }`}
                                                    >
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <div className="text-[#1F2937] truncate">
                                                                    {suggestion.name}
                                                                </div>
                ...
                {error && (
                    <div className="mt-2 text-xs text-white bg-[#E74C3C]/20 px-2 py-1 rounded">
                        {error}
                    </div>
                )}
            </div>
