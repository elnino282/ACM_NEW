      ...
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-[#333333]">
            <GitMerge className="w-5 h-5 text-[#3BA55D]" />
            Merge Plots Wizard
          </DialogTitle>
          <DialogDescription>
            Step {mergeStep} of 2:{" "}
        ...
        {mergeStep === 1 ? (
          <div className="py-4">
            ...
            <ScrollArea className="h-[300px] border border-[#E0E0E0] rounded-lg p-4">
              <div className="space-y-2">
                {plots.map((plot) => (
                  <div
                    key={plot.id}
                    className="flex items-center space-x-3 p-3 rounded-lg hover:bg-[#F5F5F5] cursor-pointer"
                    onClick={() => {
                      setSelectedPlots(
          ...
          <div className="py-4">
            <Card className="border-[#E0E0E0]">
              ...
              <CardContent className="space-y-4">
                <div>
                  <Label className="text-sm text-[#777777]">Plots to Merge</Label>
                  <div className="mt-2 space-y-1">
                    {plots
                      .filter((p) => selectedPlots.includes(p.id))
                      .map((plot) => (
                        <p key={plot.id} className="text-sm text-[#333333]">
                ...
                <div>
                  <Label className="text-sm text-[#777777]">New Plot Name</Label>
                  <Input
                    placeholder="Enter merged plot name"
                    className="mt-2 border-[#E0E0E0] focus:border-[#3BA55D]"
                  />
                </div>
                <div>
                  <Label className="text-sm text-[#777777]">Total Area</Label>
